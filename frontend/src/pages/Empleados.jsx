...
const SelectInput = ({ name, value, onChange, options, placeholder }) => {
  const currentOption = options.find(
    (opt) => (opt.id ?? opt.id_estado ?? opt.id_clinica) == value
  );
  return (
    <select name={name} value={value} onChange={onChange} className="p-2 border rounded text-sm">
      {!currentOption && value && <option value={value}>{value}</option>}
...
const Empleado = () => {
  ...
  const seleccionarEmpleado = (empleado) => {
    setEmpleadoEditando({
      ...empleado,
      id_estado: empleado.id_estado || estados.find(e => e.descripcion === empleado.estado)?.id_estado || "",
      id_clinica: empleado.id_clinica || clinicas.find(c => c.nombre_clinica === empleado.clinica)?.id_clinica || "",
  ...
  <div className="flex gap-2">
    ...
    />
      ...
      {empleadoEditando && (
  <div className="bg-yellow-100 p-4 rounded-lg shadow-md mb-6 flex gap-4">
    <div className="flex-1 grid grid-cols-1 gap-4">
      <h3 className="text-lg font-semibold mb-2">Editando: {empleadoEditando.nombre}</h3>

      <SelectInput
        name="id_estado"
        value={empleadoEditando.id_estado}
        onChange={(e) => setEmpleadoEditando({ ...empleadoEditando, id_estado: e.target.value })}
        options={estados}
      ...
      <SelectInput
        ...
        onChange={(e) => setEmpleadoEditando({ ...empleadoEditando, id_clinica: e.target.value })}
        options={clinicas}
      ...
      <input
        ...
        onChange={(e) => setEmpleadoEditando({ ...empleadoEditando, direccion: e.target.value })}
        className="p-2 border rounded text-sm"
      />
    ...
    <div className="flex-1 flex items-center justify-center">
      {empleadoEditando.foto ? (
        <img
          src={`data:image/jpeg;base64,${empleadoEditando.foto}`}
          alt="Empleado"
          className="w-64 h-64 rounded-2xl object-cover shadow-md border border-gray-200"
        />
      ) : (
        <div className="w-64 h-64 bg-gray-300 rounded-2xl shadow-md border border-gray-200 flex items-center justify-center">
          Sin foto
        </div>
      )}
    </div>
      ...
      <div className="overflow-x-auto">
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          <select value={filtroEstado} onChange={(e)=>setFiltroEstado(e.target.value)} className="p-2 border rounded text-sm">
            ...
            {estados.map(e=><option key={e.id_estado} value={e.descripcion}>{e.descripcion}</option>)}
          </select>
          <select value={filtroClinica} onChange={(e)=>setFiltroClinica(e.target.value)} className="p-2 border rounded text-sm">
            <option value="">Todas las clínicas</option>
            {clinicas.map(c=><option key={c.id_clinica} value={c.nombre_clinica}>{c.nombre_clinica}</option>)}
          </select>
        ...
        <table className="min-w-full bg-white border border-gray-200 shadow-md rounded-lg text-sm">
          <thead>
            <tr className="bg-blue-200">
              ...
              <th className="py-1 px-2 border">Fecha salida</th>
